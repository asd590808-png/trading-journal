...
section[data-testid="stSidebar"]{background-color:var(--sidebar) !important;border-right:1px solid var(--border) !important;}
section[data-testid="stSidebar"] *{color:var(--text) !important;}
section[data-testid="stSidebar"] label{font-size:0.76rem !important;color:var(--muted) !important;font-weight:300 !important;letter-spacing:0.04em !important;}
section[data-testid="stSidebar"] input,section[data-testid="stSidebar"] .stSelectbox>div>div{background-color:var(--input) !important;border:1px solid var(--border) !important;border-radius:3px !important;color:var(--text) !important;}
.stTextInput input,.stNumberInput input,.stSelectbox>div>div,.stDateInput input,[data-baseweb="input"] input{background-color:var(--input) !important;border:1px solid var(--border) !important;border-radius:3px !important;color:var(--text) !important;font-family:'Noto Sans TC',sans-serif !important;}
input::placeholder{color:var(--muted) !important;}
[data-testid="stFileUploader"]{background-color:var(--card) !important;border:1px dashed var(--border) !important;border-radius:4px !important;}
[data-testid="stFileUploader"] *{color:var(--text) !important;}
[data-baseweb="popover"],[data-baseweb="menu"],[role="listbox"]{background-color:var(--bg2) !important;border:1px solid var(--border) !important;}
...
.stButton>button{background-color:var(--slate) !important;color:#1A1C1E !important;border:none !important;border-radius:3px !important;font-family:'Noto Sans TC',sans-serif !important;font-weight:400 !important;letter-spacing:0.08em !important;padding:0.5rem 1.5rem !important;transition:all 0.2s ease !important;}
.stButton>button:hover{background-color:var(--accent) !important;box-shadow:0 2px 12px rgba(122,143,160,0.25) !important;transform:translateY(-1px) !important;}
.stDownloadButton>button{background-color:transparent !important;color:var(--slate) !important;border:1px solid var(--border) !important;border-radius:3px !important;}
.stDownloadButton>button:hover{background-color:var(--card) !important;border-color:var(--slate) !important;}
hr{border:none !important;border-top:1px solid var(--border) !important;margin:1.2rem 0 !important;}
[data-testid="stMetric"]{background-color:var(--card) !important;border:1px solid var(--border) !important;border-radius:4px !important;padding:1rem 1.2rem !important;}
[data-testid="stMetricLabel"]{font-size:0.66rem !important;letter-spacing:0.16em !important;text-transform:uppercase !important;color:var(--muted) !important;}
[data-testid="stMetricValue"]{font-family:'Noto Serif TC',serif !important;font-size:1.4rem !important;color:var(--text) !important;}
...
.sec{font-size:0.62rem;letter-spacing:0.32em;text-transform:uppercase;color:var(--muted);border-bottom:1px solid var(--border);padding-bottom:0.35rem;margin:1.6rem 0 1rem;}
...
.rw{background:var(--red-dim);border-left:3px solid var(--red);border-radius:0 3px 3px 0;padding:0.75rem 1rem;margin:0.3rem 0;font-size:0.8rem;color:#D07070;line-height:1.65;}
.rs{background:var(--grn-dim);border-left:3px solid var(--green);border-radius:0 3px 3px 0;padding:0.75rem 1rem;margin:0.3rem 0;font-size:0.8rem;color:#80B090;line-height:1.65;}
.ibox{background:var(--card);border:1px solid var(--border);border-radius:4px;padding:0.9rem 1.1rem;margin:0.5rem 0;line-height:1.8;font-size:0.85rem;}
.edit-banner{background:var(--amb-dim);border:1px solid var(--amber);border-radius:4px;padding:0.8rem 1.1rem;margin-bottom:1rem;font-size:0.82rem;color:#D0A870;line-height:1.7;}
.sym-card{background:var(--card);border:1px solid var(--border);border-left:3px solid var(--accent);border-radius:0 4px 4px 0;padding:0.65rem 1rem;margin:0.4rem 0 0.9rem;font-size:0.78rem;color:var(--text);line-height:1.65;}
.stop-hint{background:var(--amb-dim);border-left:2px solid var(--amber);border-radius:0 3px 3px 0;padding:0.45rem 0.8rem;font-size:0.72rem;color:#C0A070;margin-top:0.2rem;line-height:1.5;}
.shot-card{background:var(--card);border:1px solid var(--border);border-left:3px solid var(--accent);border-radius:0 4px 4px 0;padding:0.6rem 1rem;margin:0.3rem 0;font-size:0.78rem;color:var(--muted);line-height:1.6;}
.trade-card{background:var(--card);border:1px solid var(--border);border-radius:4px;padding:0.9rem 1.1rem;margin:0.5rem 0;line-height:1.9;}
...
.stTabs [data-baseweb="tab"]{background:transparent !important;color:var(--muted) !important;font-size:0.7rem !important;letter-spacing:0.14em !important;text-transform:uppercase !important;border-radius:0 !important;padding:0.5rem 1.4rem !important;}
...
.streamlit-expanderHeader{background-color:var(--card) !important;border-radius:4px !important;color:var(--text) !important;font-size:0.8rem !important;}
.streamlit-expanderContent{background-color:var(--card) !important;border:1px solid var(--border) !important;}
...
def load_trades(wb):
    ...
    for row in rows[1:]:
        ...
        for f in ["entry","exit","stop_usd","contracts","point_value","pnl","r_multiple"]:
            try:    t[f] = float(t[f]) if str(t.get(f,"")).strip() != "" else 0.0
            except: t[f] = 0.0
...
with tab2:
    ...
    if is_edit:
        ed = trades[edit_idx]
        def_date=date.fromisoformat(ed.get("date",today_str)); def_sym=ed.get("symbol","MNQ")
        def_dir=ed.get("direction","Long"); def_entry=float(ed.get("entry",0))
    ...
    with col_s:
        ...
        sel_sym = st.selectbox("標的", SYMBOL_LIST, index=SYMBOL_LIST.index(def_sym) if def_sym in SYMBOL_LIST else 0, label_visibility="collapsed", key=f"sym_{is_edit}")
    with col_d:
        field("交易方向","Long 做多 / Short 做空")
        sel_dir = st.selectbox("方向",["Long","Short"], index=["Long","Short"].index(def_dir) if def_dir in ["Long","Short"] else 0, label_visibility="collapsed", key=f"dir_{is_edit}")

    si = SYMBOL_DB[sel_sym]
    if sel_sym != "自訂":
        st.markdown(f'<div class="sym-card"><b style="color:var(--accent);">{si["name"]}</b><br>最小跳動：{si["tick_size"]} 點 = ${si["tick_value"]}　｜　<b>每整點 = ${si["point_value"]}</b><br><span style="color:#404558;">{si["desc"]}</span></div>', unsafe_allow_html=True)
        cur_pv = si["point_value"]
    else:
        st.markdown('<div class="sym-card" style="border-left-color:var(--red);">⚙ 自訂標的：請填入每點美金價值<br><span style="color:#404558;">查合約規格表，1 整點移動等於多少美金</span></div>', unsafe_allow_html=True)
        field("每點美金價值","NQ=20, ES=50, MNQ=2, MES=5")
        cur_pv = st.number_input("自訂點值", value=def_cpv, min_value=0.01, step=0.5, format="%.2f", label_visibility="collapsed", key="cpv_in")
    ...
    with st.form(f"form_{'edit' if is_edit else 'add'}", clear_on_submit=not is_edit):
        ...
        with col_su:
            ...
            t_stop_usd = st.number_input("停損金額", value=def_stop_usd, min_value=0.0, step=10.0, format="%.2f", label_visibility="collapsed")
            if t_stop_usd > 0 and cur_pv > 0:
                st.markdown(f'<div class="stop-hint">≈ {t_stop_usd/cur_pv:.2f} 點（每口）　|　每點 ${cur_pv}　|　自動換算 R</div>', unsafe_allow_html=True)
        with col_co:
            field("口數","1 口、2 口")
        ...
        with col_p:
            field("心理狀態","冷靜執行 = 最好")
            t_psy = st.selectbox("心理", PSY_LIST, index=PSY_LIST.index(def_psy) if def_psy in PSY_LIST else 0, label_visibility="collapsed")
        with col_n:
        ...
        if t_entry > 0 and t_exit > 0:
            ...
            st.markdown(f'<div class="ibox" style="border-left:3px solid {pc};">📊 預估損益：<b style="color:{pc};">${prev_pnl:+,.2f}</b>　｜　移動：{raw_pts:+.2f} 點　｜　口數 {t_contracts}{r_str}</div>', unsafe_allow_html=True)

        submitted = st.form_submit_button("💾  儲存修改" if is_edit else "✓  記錄此筆交易", use_container_width=True)
...
with tab3:
    ...
    if df.empty:
        ...
        pnl_v = float(sel_trade.get("pnl",0)); pnl_col = "#528060" if pnl_v>=0 else "#C05050"
        ...
        st.markdown(
            f'<div class="trade-card {cls}"><b style="color:var(--accent);">{sel_trade.get("symbol","")} {sel_trade.get("direction","")}</b>　{sel_trade.get("date","")}　× {sel_trade.get("contracts",1)} 口<br>'
            f'進場：{sel_trade.get("entry","")}　→　出場：{sel_trade.get("exit","")}<br>'
            f'停損：<b>${su_v:,.2f}</b>（≈ {stop_pts_disp}）　｜　損益：<b style="color:{pnl_col};">${pnl_v:+,.2f}</b>　｜　R：<b>{r_v:+.2f}R</b><br>'
            f'心理：{sel_trade.get("psychology","")}　｜　備註：{sel_trade.get("notes","—")}</div>',
            unsafe_allow_html=True
        )
        ...
        aw=wins["pnl"].mean() if len(wins) else 0; al=losses["pnl"].mean() if len(losses) else 0
        ...
        st.download_button("⬇  匯出 CSV（含帳戶設置）", data=csv_b,
...
st.markdown("---")
st.markdown('<div style="text-align:center;font-size:0.62rem;color:#2E3240;letter-spacing:0.18em;padding-bottom:0.8rem;">PROP FIRM TRADING JOURNAL　｜　冷靜執行，紀律為本</div>', unsafe_allow_html=True)
