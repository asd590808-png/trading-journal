"""
Prop Firm Trading Journal — Dark Edition v4
- Dark Muji theme
- Screenshot upload & lightbox viewer per trade
- Stop loss stored & shown in USD everywhere
- Cleaned history table (removed confusing columns)
...
:root {
    ...
    --border:  #2E3240;
    --red:     #C05050;
    --red-dim: rgba(192,80,80,0.14);
    --slate:   #7A8FA0;
    --green:   #528060;
    --grn-dim: rgba(82,128,96,0.14);
    --card:    #24272E;
    --input:   #2A2E37;
    --accent:  #8FAABF;
    --amber:   #B89060;
    --amb-dim: rgba(184,144,96,0.14);
}
...
.stButton > button {
    ...
    font-family: 'Noto Sans TC', sans-serif !important; font-weight: 400 !important;
    letter-spacing: 0.08em !important; padding: 0.5rem 1.5rem !important;
...
[data-testid="stMetricValue"] {
    font-family: 'Noto Serif TC', serif !important; font-size: 1.4rem !important; color: var(--text) !important;
}
...
.sym-card {
    ...
    padding:0.65rem 1rem; margin:0.4rem 0 0.9rem; font-size:0.78rem; color:var(--text); line-height:1.65;
}
...
/* ── Trade row card (history) ── */
.trade-card {
    background: var(--card); border: 1px solid var(--border);
    border-radius: 4px; padding: 0.9rem 1.1rem; margin: 0.5rem 0;
    line-height: 1.9;
}
...
/* ── Titles ── */
.mtitle { font-family:'Noto Serif TC',serif; font-weight:300; font-size:1.55rem; letter-spacing:0.18em; color:var(--text); }
.msub   { font-family:'IM Fell English',serif; font-style:italic; font-size:0.8rem; color:var(--muted); letter-spacing:0.06em; }
...
.stTabs [data-baseweb="tab"] {
    ...
    text-transform:uppercase !important; border-radius:0 !important; padding:0.5rem 1.4rem !important;
}
...
def save_screenshot(uploaded_file, trade_id: str) -> str:
    """Save uploaded image to disk, return filename."""
    ext  = os.path.splitext(uploaded_file.name)[-1].lower()
    fname = f"{trade_id}{ext}"
    fpath = os.path.join(IMG_DIR, fname)
    with open(fpath, "wb") as f:
        f.write(uploaded_file.getbuffer())
    return fname

def img_to_b64(fpath: str) -> str:
...
def get_trade_id(idx: int) -> str:
    """Stable ID string for a trade, used as screenshot filename prefix."""
...
if "view_img"    not in st.session_state: st.session_state.view_img = None  # idx of trade to show img
...
with st.sidebar:
    ...
    settings["initial_balance"]  = st.number_input("初始資金 ($)",     value=float(settings.get("initial_balance",50000)),  min_value=0.0, step=1000.0, format="%.2f", help="帳戶一開始的金額")
    settings["daily_loss_limit"] = st.number_input("每日虧損上限 ($)", value=float(settings.get("daily_loss_limit",1000)),  min_value=0.0, step=100.0,  format="%.2f", help="單日最多能虧多少，超過要停止交易")
    settings["max_drawdown"]     = st.number_input("總回撤上限 ($)",   value=float(settings.get("max_drawdown",2000)),      min_value=0.0, step=100.0,  format="%.2f", help="帳戶從最高點跌下來的最大允許金額")
    settings["profit_target"]    = st.number_input("利潤目標 ($)",     value=float(settings.get("profit_target",3000)),     min_value=0.0, step=500.0,  format="%.2f", help="達到此金額可申請出金或通過考核")
...
df = pd.DataFrame(trades) if trades else pd.DataFrame(
    columns=["date","symbol","direction","entry","exit","stop_usd","contracts",
             "point_value","pnl","r_multiple","psychology","notes","screenshot"]
)
...
with cb:
    st.markdown(
        f'<div style="text-align:right;padding-top:0.7rem;font-size:0.66rem;'
        f'letter-spacing:0.14em;color:#404558;text-transform:uppercase;">'
        f'{settings["account_name"]}</div>', unsafe_allow_html=True
    )
st.markdown("---")
...
with tab1:
    ...
    with c4:
        tc = len(df); wc = len(df[df["pnl"]>0]) if not df.empty and "pnl" in df.columns else 0
    ...
    with cpct: st.markdown(f'<div style="text-align:right;font-size:0.85rem;color:var(--accent);padding-top:0.3rem;">{profit_progress*100:.1f}%</div>', unsafe_allow_html=True)
    ...
    if not df.empty and "pnl" in df.columns and len(df) > 1:
        ...
        fig_m = go.Figure()
        fig_m.add_trace(go.Scatter(x=list(range(len(eq))), y=eq["cum"].tolist(),
...
with tab2:
    ...
    if is_edit:
        ...
        def_sym       = ed.get("symbol","MNQ")
        def_dir       = ed.get("direction","Long")
        def_entry     = float(ed.get("entry",0.0))
        def_exit      = float(ed.get("exit",0.0))
        def_stop_usd  = float(ed.get("stop_usd",0.0))
        def_contracts = int(ed.get("contracts",1))
        def_psy       = ed.get("psychology","冷靜執行")
        def_notes     = ed.get("notes","")
        def_cpv       = float(ed.get("point_value",1.0))
        existing_shot = ed.get("screenshot","")
    else:
        def_date=date.today(); def_sym="MNQ"; def_dir="Long"
        def_entry=0.0; def_exit=0.0; def_stop_usd=0.0; def_contracts=1
        def_psy="冷靜執行"; def_notes=""; def_cpv=st.session_state.cpv; existing_shot=""
    ...
    if is_edit:
        st.markdown(
            ...
            f'修改後按「儲存修改」，或按下方取消。</div>', unsafe_allow_html=True
        )
        if st.button("✕  取消編輯"):
    ...
    with col_s:
        ...
        sel_sym = st.selectbox("標的", SYMBOL_LIST,
    ...
    with col_d:
        ...
        sel_dir = st.selectbox("方向", ["Long","Short"],
    ...
    if sel_sym != "自訂":
        st.markdown(
            ...
            f'<span style="color:#404558;">{si["desc"]}</span></div>', unsafe_allow_html=True
        )
        ...
        field("每點美金價值（Point Value）","NQ=20, ES=50, MNQ=2, MES=5")
        cur_pv = st.number_input("自訂點值", value=def_cpv, min_value=0.01, step=0.5,
                                 format="%.2f", label_visibility="collapsed", key="cpv_in")
        st.session_state.cpv = cur_pv

    st.markdown("")

    form_key = f"form_{'edit' if is_edit else 'add'}"
    with st.form(form_key, clear_on_submit=not is_edit):
        ...
        with col_su:
            ...
            t_stop_usd = st.number_input(
                "停損金額 ($)", value=def_stop_usd, min_value=0.0, step=10.0,
                format="%.2f", label_visibility="collapsed"
            )
            if t_stop_usd > 0 and cur_pv > 0:
                stop_pts = t_stop_usd / cur_pv
                st.markdown(
                    f'<div class="stop-hint">≈ {stop_pts:.2f} 點（每口）　|　每點 ${cur_pv}　|　自動換算 R 乘數</div>',
                    unsafe_allow_html=True
                )
        with col_co:
            field("口數（合約張數）", "1 口、2 口")
            t_contracts = st.number_input("口數", value=def_contracts, min_value=1, step=1,
        ...
        with col_p:
            field("交易心理狀態", "冷靜執行 = 最好")
            psy_idx = PSY_LIST.index(def_psy) if def_psy in PSY_LIST else 0
            t_psy   = st.selectbox("心理", PSY_LIST, index=psy_idx, label_visibility="collapsed")
        with col_n:
            field("備註（交易思路）", "突破前高進場，止盈阻力區")
            t_notes = st.text_input("備註", value=def_notes,
                                    placeholder="簡短記錄你的想法…", label_visibility="collapsed")

        st.markdown("")

        # ── Screenshot upload ──
        field("交易截圖（選填）", "PNG / JPG / WEBP，上傳後可在歷史清單查看")
        if existing_shot:
            shot_path = os.path.join(IMG_DIR, existing_shot)
            if os.path.exists(shot_path):
                st.markdown(f'<div class="shot-card">📷 現有截圖：{existing_shot}　（上傳新圖會覆蓋）</div>', unsafe_allow_html=True)
        t_upload = st.file_uploader(
            "截圖", type=["png","jpg","jpeg","webp"],
            label_visibility="collapsed", key=f"upload_{edit_idx}_{is_edit}"
        )
        ...
        if t_entry > 0 and t_exit > 0:
            ...
            total_risk_usd = t_stop_usd * t_contracts
            r_prev         = prev_pnl / total_risk_usd if total_risk_usd > 0 else 0
            pc             = "#528060" if prev_pnl >= 0 else "#C05050"
            r_str = f'　｜　預估 R：<b>{r_prev:+.2f}R</b>' if total_risk_usd > 0 else ""
            st.markdown(
        ...
        submit_label = "💾  儲存修改" if is_edit else "✓  記錄此筆交易"
        submitted = st.form_submit_button(submit_label, use_container_width=True)

    if submitted:
        pnl            = calc_pnl(sel_sym, sel_dir, t_entry, t_exit, t_contracts, cur_pv)
        total_risk_usd = t_stop_usd * t_contracts
        r_m            = pnl / total_risk_usd if total_risk_usd > 0 else 0.0

        # Determine trade index for screenshot naming
        target_idx = edit_idx if is_edit else len(trades)
        shot_fname = existing_shot  # keep old unless new upload

        if t_upload is not None:
            shot_fname = save_screenshot(t_upload, get_trade_id(target_idx))
        ...
        record = {
            ...
            "psychology":  t_psy,
            "notes":       t_notes,
            "screenshot":  shot_fname,
        }

        if is_edit:
            data["trades"][edit_idx] = record
            st.session_state.edit_idx = None
        else:
        ...
        shot_note = "　｜　📷 截圖已儲存" if shot_fname else ""
        st.markdown(
            f'<div class="ibox" style="border-left:3px solid {pc2};">'
            f'✓ {verb}　<b>{sel_sym}</b> {sel_dir} × {t_contracts} 口　'
            f'損益：<b style="color:{pc2};">${pnl:+,.2f}</b>　R：<b>{r_m:+.2f}R</b>{shot_note}</div>',
            unsafe_allow_html=True
        )
        st.rerun()
...
with tab3:
    ...
    if df.empty:
        st.markdown('<div class="ibox" style="text-align:center;color:#404558;padding:2.5rem;">尚無交易紀錄<br><small>前往「新增 / 編輯」開始記錄</small></div>', unsafe_allow_html=True)
    else:
        # ── Clean display table ──
        # Only show columns that are meaningful to the user
        display_cols_def = [
            ("date",       "日期"),
            ("symbol",     "標的"),
            ("direction",  "方向"),
            ("entry",      "進場價"),
            ("exit",       "出場價"),
            ("stop_usd",   "停損金額 ($)"),   # USD amount — renamed clearly
            ("contracts",  "口數"),
            ("pnl",        "損益 ($)"),
            ("r_multiple", "R 乘數"),
            ("psychology", "心理狀態"),
        ...
        show_raw = df.copy().sort_values("date", ascending=False).reset_index(drop=True)

        # Format columns
        if "pnl" in show_raw.columns:
            show_raw["pnl"] = show_raw["pnl"].apply(lambda x: f"+${x:,.2f}" if x>=0 else f"-${abs(x):,.2f}")
        if "r_multiple" in show_raw.columns:
            show_raw["r_multiple"] = show_raw["r_multiple"].apply(lambda x: f"{x:+.2f}R")
        if "stop_usd" in show_raw.columns:
            show_raw["stop_usd"] = show_raw["stop_usd"].apply(lambda x: f"${x:,.2f}" if x > 0 else "—")

        col_rename = {c: lbl for c, lbl in display_cols_def}
        avail_cols = [c for c,_ in display_cols_def if c in show_raw.columns]
        show_final = show_raw[avail_cols].rename(columns=col_rename)
        st.dataframe(show_final, use_container_width=True, hide_index=True)

        # ── Expanded per-trade view with screenshot ──
        st.markdown('<div class="sec">逐筆查看 ／ 截圖</div>', unsafe_allow_html=True)
        st.markdown('<div class="flabel">選擇要查看的紀錄</div>', unsafe_allow_html=True)
        ...
        for orig_i, t in sorted_trades:
            pnl_v = t.get("pnl", 0)
            pnl_s = f"+${pnl_v:,.0f}" if pnl_v >= 0 else f"-${abs(pnl_v):,.0f}"
            has_img = "📷" if t.get("screenshot") and os.path.exists(os.path.join(IMG_DIR, t["screenshot"])) else "  "
            label_list.append(f"{has_img} #{orig_i+1}　{t.get('date','')}　{t.get('symbol','')} {t.get('direction','')}　{pnl_s}")
            orig_indices.append(orig_i)

        sel_label = st.selectbox("選擇紀錄", label_list, label_visibility="collapsed")
        sel_pos   = label_list.index(sel_label)
        sel_orig  = orig_indices[sel_pos]
        sel_trade = trades[sel_orig]

        # Detail card
        pnl_v   = sel_trade.get("pnl", 0)
        pnl_col = "#528060" if pnl_v >= 0 else "#C05050"
        r_v     = sel_trade.get("r_multiple", 0)
        su_v    = sel_trade.get("stop_usd", 0)
        pv_v    = sel_trade.get("point_value", 1)
        stop_pts_display = f"{su_v/pv_v:.2f} 點" if su_v > 0 and pv_v > 0 else "—"

        st.markdown(
            f'<div class="trade-card {"trade-card-win" if pnl_v>=0 else "trade-card-loss"}">'
            ...
            f'停損：<b>${su_v:,.2f}</b>（≈ {stop_pts_display}）　｜　'
        ...
        # Screenshot viewer
        shot_fname = sel_trade.get("screenshot","")
        shot_path  = os.path.join(IMG_DIR, shot_fname) if shot_fname else ""

        if shot_fname and os.path.exists(shot_path):
            with st.expander("📷  查看交易截圖", expanded=False):
                try:
                    b64 = img_to_b64(shot_path)
                    ext = os.path.splitext(shot_fname)[-1].lower().replace(".","")
                    mime = "jpeg" if ext in ("jpg","jpeg") else ext
                    st.markdown(
                        f'<div class="lb-wrap">'
                        f'<img src="data:image/{mime};base64,{b64}" alt="trade screenshot"/>'
                        f'</div>',
                        unsafe_allow_html=True
                    )
                    st.download_button(
                        "⬇  下載截圖", data=open(shot_path,"rb").read(),
                        file_name=shot_fname, mime=f"image/{mime}"
                    )
                except Exception as e:
                    st.warning(f"無法載入截圖：{e}")
        else:
            st.markdown('<div class="shot-card" style="border-left-color:var(--muted);">此筆紀錄尚未上傳截圖。可進入「編輯」後上傳。</div>', unsafe_allow_html=True)
        ...
        with dc:
            if st.button("🗑  刪除此筆", use_container_width=True):
                ...
                del_shot = trades[sel_orig].get("screenshot","")
                if del_shot:
                    del_path = os.path.join(IMG_DIR, del_shot)
                    if os.path.exists(del_path):
                        os.remove(del_path)
                data["trades"].pop(sel_orig)
                save_data(data)
                st.session_state.data  = data
                st.session_state.edit_idx = None
                st.rerun()
        ...
        exp = raw_df.copy()
        # Remove internal / confusing columns before export
        for drop_c in ["point_value","stop_points"]:
            if drop_c in exp.columns: exp = exp.drop(columns=[drop_c])
        ...
        st.markdown('<div style="font-size:0.72rem;color:#404558;margin-top:0.3rem;">下載後直接上傳給 Claude，請他幫你做深度交易覆盤分析 📊</div>', unsafe_allow_html=True)
...
with tab4:
    if df.empty or len(df) < 2:
        ...
        raw = pd.DataFrame(trades).sort_values("date").reset_index(drop=True)
        ...
        fig_eq = go.Figure()
        fig_eq.add_trace(go.Scatter(x=raw["trade_num"].tolist(), y=raw["cum"].tolist(),
                                    fill=None, mode="lines", line=dict(color="rgba(0,0,0,0)"), showlegend=False))
        fig_eq.add_trace(go.Scatter(x=raw["trade_num"].tolist(), y=peak.tolist(),
                                    fill="tonexty", fillcolor="rgba(192,80,80,0.12)", mode="none", name="回撤區間"))
        fig_eq.add_trace(go.Scatter(x=raw["trade_num"].tolist(), y=raw["cum"].tolist(),
                                    mode="lines+markers", line=dict(color="#7A8FA0",width=2),
                                    marker=dict(size=4,color="#8FAABF"), name="帳戶淨值"))
        fig_eq.add_hline(y=settings["initial_balance"], line_dash="dot", line_color="#404558", line_width=1,
                         annotation_text="初始資金", annotation_position="right", annotation_font_color="#60646C")
        fig_eq.add_hline(y=settings["initial_balance"]+settings["profit_target"],
                         line_dash="dash", line_color="#528060", line_width=1,
                         annotation_text="利潤目標", annotation_position="right", annotation_font_color="#528060")
        fig_eq.add_hline(y=settings["initial_balance"]-settings["max_drawdown"],
                         line_dash="dash", line_color="#C05050", line_width=1,
                         annotation_text="最大回撤", annotation_position="right", annotation_font_color="#C05050")
        fig_eq.update_layout(
            height=360, margin=dict(l=10,r=80,t=10,b=30),
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#22252A",
        ...
        with cl:
            ...
            pc = raw["psychology"].value_counts().reset_index()
            pc.columns = ["label","count"]
            palette = ["#7A8FA0","#8FAABF","#6A7F90","#528060","#A0B5C5","#C05050","#B07070"]
            fig_ps = go.Figure(go.Bar(
                x=pc["count"].tolist(), y=pc["label"].tolist(), orientation="h",
                marker_color=palette[:len(pc)],
                text=pc["count"].tolist(), textposition="inside",
                textfont=dict(color="#1A1C1E", size=10),
            ))
            fig_ps.update_layout(height=280, margin=dict(l=10,r=10,t=10,b=10),
                                 paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                                 font=dict(family="Noto Sans TC", size=10, color="#60646C"),
                                 xaxis=dict(showgrid=False,showticklabels=False,zeroline=False),
        ...
        with cr:
            ...
            rv = raw["r_multiple"].tolist()
            rc = ["#528060" if r>=0 else "#C05050" for r in rv]
            ...
            fig_r.update_layout(height=280, margin=dict(l=10,r=10,t=10,b=20),
                                 paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                                 font=dict(family="Noto Sans TC", size=10, color="#60646C"),
                                 xaxis=dict(title="交易筆數", showgrid=False, zeroline=False),
                                 yaxis=dict(title="R 乘數", showgrid=True, gridcolor="#2A2E37", zeroline=False),
                                 showlegend=False)
        ...
        fig_pp = go.Figure(go.Bar(
            x=ppnl["psychology"].tolist(), y=ppnl["avg"].tolist(), marker_color=bc, opacity=0.85,
            text=[f"${v:,.0f}" for v in ppnl["avg"]], textposition="outside",
            textfont=dict(size=9, color="#60646C"),
        ))
        fig_pp.add_hline(y=0, line_color="#404558", line_width=1)
        fig_pp.update_layout(height=250, margin=dict(l=10,r=10,t=20,b=20),
                              paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                              font=dict(family="Noto Sans TC", size=10, color="#60646C"),
                              xaxis=dict(showgrid=False, zeroline=False),
                              yaxis=dict(showgrid=True, gridcolor="#2A2E37", zeroline=False, tickformat="$,.0f"),
                              showlegend=False)
